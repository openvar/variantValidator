...
#the class is made. 
 ...
 # for that key.    
    def add_entry(self, term, description, SO_number, display_term, impact):
        self.term_definitions[term] = [description, SO_number, display_term, impact]

#Calling an instance of the Ensembl_reference_dict class and populating the
#dictionary.
Ensembl_reference = Ensembl_reference_dict()
Ensembl_reference.add_entry("frameshift_variant", "A sequence variant which causes a disruption of the translational reading frame, because the number of nucleotides inserted or deleted is not a multiple of three", "SO:0001589", "Frameshift variant", "High")
Ensembl_reference.add_entry("stop_gained", "A sequence variant whereby at least one base of a codon is changed, resulting in a premature stop codon, leading to a shortened transcript", "SO:00015872", "Stop gained", "High")
Ensembl_reference.add_entry("stop_lost", "A sequence variant where at least one base of the terminator codon (stop) is changed, resulting in an elongated transcript", "SO:0001578", "Stop lost", "High")
Ensembl_reference.add_entry("start_lost", "A codon variant that changes at least one base of the canonical start codon","SO:0002012", "Start lost", "High")
Ensembl_reference.add_entry("transcript_amplification", "A feature amplification of a region containing a transcript", "SO:0001889", "Transcript amplification", "High")
Ensembl_reference.add_entry("missense_variant", "A sequence variant, that changes one or more bases, resulting in a different amino acid sequence but where the length is preserved", "SO:0001583", "Missense variant", "Medium")
