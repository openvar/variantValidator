...
def convert_indel_to_expanded_repeat(variant, validator, genomic_reference=None):
    """
    ...
    if any(key in variant for key in ['ins', 'del', 'dup', '=']):
        ...
        reference, indel_info = parts

        # Process insertions
        if "ins" in indel_info:
            position, sequence = indel_info.split('ins')
            variant_type = 'ins'
        ...
        # Process identity cases (=)
            ...
            indel_info = indel_info.replace('=', "")
            position, sequence = re.split(r'(?<=\d)(?=[A-Za-z])', indel_info)
            start, end = map(int, position.split('_'))
            repeated_unit = decipher_repeated_unit(sequence)
        ...
        elif "dup" in indel_info:
            position, sequence = indel_info.split('dup')
        ...
        # Process deletions
        elif "del" in indel_info:
            position, sequence = indel_info.split('del')
            start, end = map(int, position.split('_'))
            repeated_unit = decipher_repeated_unit(sequence)
            reference_start = decipher_start_of_full_reference_repeated_sequence(reference, repeated_unit, start, validator)
            reference_end = decipher_end_of_full_reference_repeated_sequence(reference, repeated_unit, start, validator)
...
if __name__ == '__main__':
    ...
    result = convert_indel_to_expanded_repeat("NM_002111.8:c.54_116=", validator, genomic_reference="NC_000004.11")
    print(result)
