...
class GapMapper(object):
                ...
                if disparity_deletion_in[0] == 'transcript':
                    ...
                    if (re.search(r'\+', str(tx_hgvs_not_delins.posedit.pos.start)) or re.search(r'\-',
                        ...
                        if tx_hgvs_not_delins.posedit.pos.start.offset == 0 and tx_hgvs_not_delins.posedit.pos.end.offset == 0:
                            ...
                            g1 = self.validator.nr_vm.t_to_g(c1, self.variant.hgvs_genomic.ac)
                            g3 = self.validator.nr_vm.t_to_g(c1, self.variant.hgvs_genomic.ac)
                            g2 = self.validator.vm.t_to_g(c1, self.variant.hgvs_genomic.ac)
                        ...
                        if re.search(r'\+', str(tx_hgvs_not_delins.posedit.pos.start)) and not re.search(
                            ...
                            c1.posedit.edit.alt = ''
                            if orientation != -1:
                                g1 = self.validator.vm.t_to_g(c1, self.variant.hgvs_genomic.ac)
                                g2 = self.validator.vm.t_to_g(c2, self.variant.hgvs_genomic.ac)
                                g1.posedit.edit.alt = g1.posedit.edit.ref
                            else:
                                g1 = self.validator.vm.t_to_g(c2, self.variant.hgvs_genomic.ac)
                                g2 = self.validator.vm.t_to_g(c1, self.variant.hgvs_genomic.ac)
    ...
    # def move_tx_end_base_to_next_non_offset(validator, variant, rn_tx_hgvs_not_delins):
    #     # move tx end base back to next available non-offset base
    #     rn_tx_hgvs_not_delins.posedit.pos.end.offset = 0
    #     # Delete the ref
    #     rn_tx_hgvs_not_delins.posedit.edit.ref = ''
