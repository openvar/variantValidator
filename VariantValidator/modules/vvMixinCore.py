        ...
        try:
            ...
            for my_variant in self.batch_list:
                ...
                try:
                    ...
                    """
                    input_parses = self.hp.parse_hgvs_variant(input)
                    if input_parses.type == 'g':
                        ...
                        if re.search(r'\*', str(input_parses)) or re.search(r'c.\-', str(input_parses)):
                                    ...
                                    if re.search('bounds', error) or re.search('intronic variant', error):
                                        ...
                                        if re.search('bounds', str(error)):
                                            try:
                                                ...
                                                lose_vm = hgvs.variantmapper.VariantMapper(self.hdp,
                                                                                           replace_reference=True,
                                                                                           prevalidation_level=None
                                                                                           )


                                                report_gen = self.myevm_t_to_g(input_parses, no_norm_evm,
                                                                                  primary_assembly, hn)
                            ...
                            try:
                                ...
                                if re.search('bounds', error):
                                    try:
                                        report_gen = self.myevm_t_to_g(input_parses, no_norm_evm, primary_assembly,
                                                                          hn)
                                    except hgvs.exceptions.HGVSError as e:
                                        fn.exceptPass()
                                    else:
                            ...
                            try:
                                output = self.noreplace_myevm_t_to_g(input_parses, evm, self.hdp, primary_assembly, self.vm, hn,
                                                                        self.hp, self.sf, no_norm_evm)
                            except hgvs.exceptions.HGVSDataNotAvailableError as e:
                        ...
                        if re.search(r'\d\-', str(input_parses)) or re.search(r'\d\+', str(input_parses)):
                            ...
                            try:
                                ...
                                if re.search('bounds', error):
                                    ...
                                    try:
                                        test_g = self.myevm_t_to_g(input_parses, no_norm_evm, primary_assembly,
                                                                      hn)
                    ...
                    if format_type == ':m.' or re.match('NC_012920.1', str(input_parses.ac)) or re.match('NC_001807.4',
                        ...
                        try:
                            ...
                            if len(rel_var) == 0:
                                my_variant.genomic_g = fn.valstr(hgvs_mito)
                                my_variant.description = 'Homo sapiens mitochondrion, complete genome'
                    ...
                    if format_type == ':p.':
                        ...
                        if error != 'false':
                            ...
                            if alt_aln_method != 'genebuild':
                                ...
                                record = self.entrez_efetch(db="nuccore", id=accession, rettype="gb", retmode="text")
                                try:
                                ...
                                my_variant.warnings += ': ' + str(reason) + ': ' + str(error)
                                my_variant.protein = str(hgvs_object)
                    ...
                    # Gene symbol
                    logger.trace("Handled mitochondrial variants", my_variant)
                    """
                    ...
                    if (format_type != ':g.'):
                        ...
                        if error != 'false':
                            ...
                            my_variant.warnings += ': ' + str(reason)
                            logger.warning(str(reason) + ": " + str(error))
                            continue
                        else:
                        ...
                        # Refseq accession
                        tx_for_gene = self.tx_for_gene(hgnc, self.hdp)
                        refseq_ac = self.ng_extract(tx_for_gene)

                        # Additional gene info
                        gene_info = self.hdp.get_gene_info(hgnc)
                    ...
                    if (format_type == ':g.'):
                        ...
                        if len(rel_var) != 0 and re.match('NG_', str(hgvs_genomic.ac)):
                            for var in rel_var:
                                hgvs_coding_variant = self.hp.parse_hgvs_variant(var)
                                try:
                                    hgvs_genomic = self.myevm_t_to_g(hgvs_coding_variant, no_norm_evm,
                                                                        primary_assembly, hn)
                                except hgvs.exceptions.HGVSError as e:
                                    try_rel_var = []
                                else:
                        ...
                        if len(rel_var) == 0:
                            ...
                            if rsg.search(formatted_variant):
                                ...
                                if refseqgene_data['valid'] == 'true':
                                    ...
                                    my_variant.warnings += ': ' + formatted_variant + ' automapped to genome position ' + str(
                                        input)
                                    query = variant.Variant(my_variant.original, quibble=input, warnings=my_variant.warnings, primary_assembly=my_variant.primary_assembly, order=ordering)
                            ...
                            # Chromosome build is not supported or intergenic???
                                ...
                                sfm = vvChromosomes.supported_for_mapping(hgvs_genomic.ac, primary_assembly)
                                if sfm == 'true':
                                    try:
                                        ...
                                        refseqgene_data = self.chr_to_rsg(hgvs_genomic, hn, self.vr)
                                        rsg_data = ''
                                        # Example {'gene': 'NTHL1', 'hgvs_refseqgene': 'NG_008412.1:g.3455_3464delCAAACACACA', 'valid': 'true'}
                                        for data in refseqgene_data:
                                            if data['valid'] == 'true':
                                                data['hgvs_refseqgene'] = self.hp.parse_hgvs_variant(data['hgvs_refseqgene'])
                                        ...
                                        my_variant.warnings += ': ' + str(error)
                                        my_variant.genomic_g = fn.valstr(hgvs_genomic)
                                        my_variant.genomic_r = str(rsg_data.split('(')[0])
                                        logger.warning(str(error))
                                        continue
                                else:
                            ...
                            if re.match('NG_', str(stash_input)):
                                c = self.hp.parse_hgvs_variant(rel_var[0])
                                ...
                                stash_input = self.myevm_t_to_g(c, no_norm_evm, primary_assembly, hn)
                            if re.match('NC_', str(stash_input)) or re.match('NT_', str(stash_input)) or re.match('NW_',
                                                                                                                  str(
                                                                                                                          stash_input)):
                                try:
                                    hgvs_stash = self.hp.parse_hgvs_variant(stash_input)
                                except:
                            ...
                            for var in rel_var:
                                ...
                                saved_hgvs_coding = self.hp.parse_hgvs_variant(var)
                                ...
                                if intronic_variant != 'true':
                                    ...
                                    if stored_hgvs_not_delins != '':
                                            ...
                                            if re.search('dup', hgvs_genomic_5pr.posedit.edit.type) or re.search('ins',
                                                ...
                                                if ((
                                                    ...
                                                    if re.search('dup', str(hgvs_genomic_5pr.posedit.edit)):
                                                        ...
                                                        ref_bases = self.sf.fetch_seq(str(hgvs_not_delins.ac), start, end)
                                        ...
                                        if (re.search(r'\+', str(rn_tx_hgvs_not_delins.posedit.pos.start)) or re.search(r'\-',
                                            ...
                                            hgvs_not_delins = self.myevm_t_to_g(test_tx_var, no_norm_evm,
                                                                                   primary_assembly, hn)
                                        ...
                                        if len(hgvs_not_delins.posedit.edit.ref) < len(
                                            ...
                                            hgvs_stash_t = self.vm.g_to_t(stash_hgvs_not_delins, saved_hgvs_coding.ac)
                                            if len(stash_hgvs_not_delins.posedit.edit.ref) > len(
                                                ...
                                                try:
                                                    ...
                                                    disparity_deletion_in = ['transcript', gap_length]
                                                    try:
                                                        tx_hgvs_not_delins = self.vm.c_to_n(hgvs_stash_t)
                                                    except:
                                    ...
                                    # GAP IN THE CHROMOSOME
                                        ...
                                        stash_hgvs_not_delins_left = self.hp.parse_hgvs_variant(
                                            stash_ac + ':' + hgvs_stash.type + '.' + str(
                                                stash_pos_left) + '_' + stash_end_left + 'del' + stash_ref_left + 'ins' + stash_alt_left)
                                        # Map in-situ to the transcript left and right
                                        try:
                                            tx_hard_right = self.vm.g_to_t(stash_hgvs_not_delins_right, saved_hgvs_coding.ac)
                                        except Exception as e:
                                            ...
                                            if str(normalize_stash_right.posedit) == str(stash_hgvs_not_delins.posedit):
                                                tx_hard_right = saved_hgvs_coding
                                        try:
                                            tx_hard_left = self.vm.g_to_t(stash_hgvs_not_delins_left, saved_hgvs_coding.ac)
                                        except Exception as e:
                            ...
                            for c_description in rel_var:
                                query = variant.Variant(my_variant.original, quibble=str(c_description), warnings=my_variant.warnings, primary_assembly=my_variant.primary_assembly, order=ordering)
                                self.batch_list.append(query)
                    ...
                    if format_type == ':c.' or format_type == ':n.':
                        ...
                        rec_var = ''
                        try:
                            to_g = self.myevm_t_to_g(obj, no_norm_evm, primary_assembly, hn)
                            genomic_ac = to_g.ac
                        except hgvs.exceptions.HGVSDataNotAvailableError as e:
                        ...
                        if plus.search(input) or minus.search(input):
                            ...
                            try:
                                ...
                                if re.match('Unsupported normalization of variants spanning the exon-intron boundary',
                                    ...
                                    my_variant.warnings += ': ' + str(caution) + ': ' + str(
                                        automap)
                            ...
                            if dl.search(formatted_variant):
                                ...
                                if pat_r.search(trapped_input):
                                    ...
                                    if post_var.posedit.pos.start.base != test.posedit.pos.start.base or post_var.posedit.pos.end.base != test.posedit.pos.end.base:
                                        ...
                                        for accessions in cp_rel:
                                            error = 'false'
                                            hgvs_vt = self.hp.parse_hgvs_variant(str(accessions))
                                            try:
                                                tx_id_info = self.hdp.get_tx_identity_info(str(hgvs_vt.ac))
                                            except hgvs.exceptions.HGVSError as e:
                                                error = str(e)
                                            if error != 'false':
                                                ...
                                                # Get hgnc Gene name from command
                                                data = self.hgnc_rest(path="/search/prev_symbol/" + tx_id_info[6])
                                                if data['error'] != 'false':
                                                    error = data['error']
                                                    my_variant.warnings += ': ' + str(error)
                                                    logger.warning(str(error))
                                                ...
                                                for accessions in cp_rel:
                                                    error = 'false'
                                                    hgvs_vt = self.hp.parse_hgvs_variant(str(accessions[1]))
                                                    try:
                                                        tx_id_info = self.hdp.get_tx_identity_info(str(hgvs_vt.ac))
                                                    except hgvs.exceptions.HGVSError as e:
                                                        error = str(e)
                                        ...
                                        my_variant.write = False
                                        # Set the values and append to batch_list
                                        query = variant.Variant(my_variant.original, quibble=fn.valstr(hgvs_vt), warnings=automap, primary_assembly=my_variant.primary_assembly, order=ordering)
                                        self.batch_list.append(query)
                                ...
                                # Coding
                                    ...
                                    if post_var.posedit.pos.start.base != test.posedit.pos.start.base or post_var.posedit.pos.end.base != test.posedit.pos.end.base:
                                        ...
                                        # automapping of variant completed
                                        automap = trapped_input + ' automapped to ' + str(post_var)
                                        my_variant.warnings += str(caution) + ': ' + str(automap)
                                        relevant = "Select the automapped transcript and click Submit to analyse"
                                        rel_var = []
                                        rel_var.append(post_var)
                                        # Add gene symbols to the link
                                        ...
                                        for accessions in cp_rel:
                                            ...
                                            if error != 'false':
                                                ...
                                                for accessions in cp_rel:
                                                    error = 'false'
                                                    hgvs_vt = self.parse_hgvs_variant(str(accessions[1]))
                                                    try:
                                                        tx_id_info = self.hdp.get_tx_identity_info(str(hgvs_vt.ac))
                                                    except hgvs.exceptions.HGVSError as e:
                                                        error = str(e)
                                    ...
                                    if post_var.posedit.pos.start.base != test.posedit.pos.start.base or post_var.posedit.pos.end.base != test.posedit.pos.end.base:
                                        ...
                                        for accessions in cp_rel:
                                            ...
                                            if error != 'false':
                                                ...
                                                # Get hgnc Gene name from command
                                                data = self.va_func.hgnc_rest(path="/search/prev_symbol/" + tx_id_info[6])
                                                if data['error'] != 'false':
                                                    error = data['error']
                                                    my_variant.warnings += ': ' + str(error)
                                                    logger.warning(str(error))
                        ...
                        # If cck not true
                            ...
                            hgvs_rna_input = self.hp.parse_hgvs_variant(
                            ...
                            if plus.search(input) or minus.search(input):
                                ...
                                if es.search(str(to_g)):
                                    ...
                                    hgvs_inp = self.myevm_g_to_t(evm, to_g, tx_ac=obj.ac)
                                    to_g = hn.normalize(to_g)
                                    hgvs_otp = self.myevm_g_to_t(evm, to_g, tx_ac=obj.ac)
                                    tx_ac = ''
                            else:
                                # Set variants pre and post RNA norm
                                hgvs_inp = self.hp.parse_hgvs_variant(inp)
                                try:
                            ...
                            if query != test:
                                ...
                                my_variant.write = False
                                # Set the values and append to batch_list
                                query = variant.Variant(my_variant.original, quibble=fn.valstr(hgsv_vt), warnings=automap, primary_assembly=my_variant.primary_assembly, order=ordering)
                                self.batch_list.append(query)
                        ...
                        if valid == 'false':
                            ...
                            error = self.validateHGVS(genomic_validation)
                            if error == 'false':
                                valid = 'true'
                            else:
                        ...
                        if valid == 'true':
                            ...
                            hgvs_rna = self.hgvs_c_to_r(hgvs_rna)
                            rna = str(hgvs_rna)

                            # Genomic sequence
                            hgvs_genomic = self.myevm_t_to_g(hgvs_coding, no_norm_evm, primary_assembly, hn)
                            ...
                            if gap_compensation is True:
                                ...
                                try:
                                    ...
                                    stash_genomic = self.myvm_t_to_g(test_stash_tx_right, hgvs_genomic.ac, no_norm_evm, hn)
                                ...
                                try:
                                    ...
                                    stash_genomic = self.myvm_t_to_g(test_stash_tx_left, hgvs_genomic.ac, no_norm_evm, hn)
                                    # Stash the outputs if required
                                    # test variants = NC_000006.11:g.90403795G= (causes double identity)
                                    ...
                                    if intronic_variant != 'true':
                                            ...
                                            if len(hgvs_not_delins.posedit.edit.ref) < len(
                                                ...
                                                for internal_possibility in hgvs_genomic_possibilities:
                                                    ...
                                                    if internal_possibility.posedit.edit.type == 'ins':
                                                        ins_ref = self.sf.fetch_seq(internal_possibility.ac,
                                        ...
                                        if str(to_test.posedit.edit) != str(hgvs_coding.posedit.edit):
                                            ...
                                            if hgvs_coding.posedit.edit.type == 'identity' and to_test.posedit.edit.type == 'identity':
                                                ...
                                                continue
                                        # Update hgvs_genomic
                                        hgvs_genomic = self.myvm_t_to_g(hgvs_refreshed_variant, hgvs_genomic.ac,
                                                                           no_norm_evm,hn)
                                ...
                                if auto_info != '':
                                    ...
                                    my_variant.warnings += ': ' + str(auto_info)
                            ...
                            # Attempt 1 = UTA
                            sequences_for_tx = self.hdp.get_tx_mapping_options(hgvs_coding.ac)
                            recovered_rsg = []
                            ...
                            tx_id_info = self.hdp.get_tx_identity_info(hgvs_coding.ac)
                            uta_gene_symbol = tx_id_info[6]
                            tx_for_gene = self.hdp.get_tx_for_gene(uta_gene_symbol)
                            ...
                            if update != '':
                                ...
                                # Updated reference sequence
                                    ...
                                    if re.search('does not agree with reference sequence', str(error)):
                                        ...
                                        pass
                                updated_transcript_variant = hgvs_updated
                                my_variant.warnings += ': ' + 'A more recent version of the selected reference sequence ' + hgvs_coding.ac + ' is available (' + updated_transcript_variant.ac + ')' + ': ' + str(
