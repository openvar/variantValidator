...
-- ============================================================
-- VariantValidator dbSNP schema (final, lean version)
-- ============================================================
...
CREATE TABLE `dbsnp_grch37` (
  ...
  `ref_hash` BINARY(16)
    GENERATED ALWAYS AS (UNHEX(MD5(`ref_blob`))) STORED,

  `alt_hash` BINARY(16)
    GENERATED ALWAYS AS (UNHEX(MD5(`alt_blob`))) STORED,

  PRIMARY KEY (`id`),

  UNIQUE KEY `uniq_variant`
    (`rs_int`, `contig_id`, `pos`, `ref_hash`, `alt_hash`),
