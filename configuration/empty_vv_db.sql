...
--
-- Table structure for table `dbsnp_grch37 and 38`
--

CREATE TABLE `contig_lookup_grch37` (
  `contig_id` SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `contig_name` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`contig_id`),
  UNIQUE KEY `u_contig_name` (`contig_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE `contig_lookup_grch38` (
  `contig_id` SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `contig_name` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`contig_id`),
  UNIQUE KEY `u_contig_name` (`contig_name`)
...
CREATE TABLE `rsid_lookup` (
  `rs_int` BIGINT UNSIGNED NOT NULL,
  `rs_id` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`rs_int`),
  UNIQUE KEY `u_rsid` (`rs_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE `dbsnp_grch37` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `rs_int` BIGINT UNSIGNED NOT NULL,
  `contig_id` SMALLINT UNSIGNED NOT NULL,
  `pos` INT UNSIGNED NOT NULL,
  `ref_blob` MEDIUMBLOB NOT NULL,
  `alt_blob` MEDIUMBLOB NOT NULL,
  `ref_hash` BINARY(16) GENERATED ALWAYS AS (UNHEX(MD5(ref_blob))) STORED,
  `alt_hash` BINARY(16) GENERATED ALWAYS AS (UNHEX(MD5(alt_blob))) STORED,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_variant` (`rs_int`, `contig_id`, `pos`, `ref_hash`, `alt_hash`),
  KEY `lookup_rsid` (`rs_int`),
  KEY `lookup_contig_pos` (`contig_id`, `pos`),
  KEY `lookup_hashes` (`ref_hash`, `alt_hash`)
) ENGINE=InnoDB ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=8 DEFAULT CHARSET=utf8mb4;

CREATE TABLE `dbsnp_grch38` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `rs_int` BIGINT UNSIGNED NOT NULL,
  `contig_id` SMALLINT UNSIGNED NOT NULL,
  `pos` INT UNSIGNED NOT NULL,
