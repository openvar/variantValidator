        ...
        stage("Build and Run VariantValidator") {
            steps {
                script {
                    ...
                    variantValidatorContainer.run("-e USE_VOLUME=true -v $PWD:$WORKDIR -v $DATA_VOLUME:/usr/local/share:rw -d --name variantvalidator --network $DOCKER_NETWORK")
                ...
                script {
                    ...
                    for (int attempt = 1; attempt <= 5; attempt++) {
                        ...
                        if (exitCode == 0) {
                            ...
                            sh 'ls -l && docker exec variantvalidator pytest --cov-report=term --cov=VariantValidator tests/ && ls -l && codecov -t $CODECOV_TOKEN -b ${BRANCH_NAME}'
