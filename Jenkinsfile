        ...
        stage("Build and Run SeqRepo") {
            steps {
                script {
                    ...
                    seqRepoContainer.run("--network $DOCKER_NETWORK --privileged -v $DATA_VOLUME:/usr/local/share:rw") // Use DATA_VOLUME as shared directory
        ...
        stage("Build and Run VariantValidator") {
            steps {
                script {
                    ...
                    variantValidatorContainer.run("--privileged -v $DATA_VOLUME:/usr/local/share:rw -d --name variantvalidator-${CONTAINER_SUFFIX} --network $DOCKER_NETWORK") // Use DATA_VOLUME as shared directory
