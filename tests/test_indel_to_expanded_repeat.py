...
from VariantValidator.modules import indel_to_expanded_repeat
from VariantValidator.modules.indel_to_expanded_repeat import convert_indel_to_expanded_repeat, VariantFormatError
...
class TestExpandedRepeatConversion(unittest.TestCase):
    ...
    def test_expanded_repeat_polyA_variant(self):
        ...
        result = convert_indel_to_expanded_repeat(variant, self.validator)
    ...
    def test_invalid_variant_format_raises(self):
        with self.assertRaises(VariantFormatError):
            convert_indel_to_expanded_repeat("invalid_variant_string", self.validator)
    ...
    def test_missing_colon_format_raises(self):
        ...
        with self.assertRaises(exceptions.HGVSParseError):
            convert_indel_to_expanded_repeat(variant, self.validator)

    def test_no_indel_returns_none(self):
    ...
    def test_decipher_repeated_unit_non_repeat(self):
        ...
        result = indel_to_expanded_repeat.decipher_repeated_unit("ACGT")
    ...
    def test_decipher_repeated_unit_single_base_repeat(self):
        ...
        result = indel_to_expanded_repeat.decipher_repeated_unit("AAAAAAAA")
        self.assertEqual(result, "A")

    def test_start_of_repeat_invalid_repeated_unit(self):
        with self.assertRaises(indel_to_expanded_repeat.RepeatedUnitError):
            indel_to_expanded_repeat.decipher_start_of_full_reference_repeated_sequence("REF", "", 100, self.validator)

    def test_start_of_repeat_invalid_start_position(self):
        with self.assertRaises(indel_to_expanded_repeat.StartPositionError):
            indel_to_expanded_repeat.decipher_start_of_full_reference_repeated_sequence("REF", "A", -10, self.validator)

    def test_end_of_repeat_invalid_repeated_unit(self):
        with self.assertRaises(indel_to_expanded_repeat.RepeatedUnitError):
            indel_to_expanded_repeat.decipher_end_of_full_reference_repeated_sequence("REF", "", 100, self.validator)

    def test_end_of_repeat_invalid_start_position(self):
        with self.assertRaises(indel_to_expanded_repeat.StartPositionError):
            indel_to_expanded_repeat.decipher_end_of_full_reference_repeated_sequence("REF", "A", 0, self.validator)

    def test_expanded_repeat_coding_variant_negative_positions(self):
        variant = "NM_004006.2:c.-3_1="
        expected = "NM_004006.2:c.-3_1A[4]"

        result = convert_indel_to_expanded_repeat(variant, validator=self.validator)
    ...
    def test_intronic_coding_sense_strand(self):
        ...
        result = convert_indel_to_expanded_repeat(variant, validator=self.validator, genomic_reference="NC_000007.13")
        self.assertEqual(result, expected)
    ...
    def test_intronic_coding_antisense_strand(self):
        ...
        result = convert_indel_to_expanded_repeat(variant, validator=self.validator, genomic_reference="NC_000017.10")
        self.assertEqual(result, expected)
